...
const SignupForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (touched.fullName && !isValidName(fullName)) {
  ...
  }, [fullName, email, password, confirmPassword, touched, passwordStrength, passwordsMatch]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate API call
    setTimeout(() => {
      // Success scenario - in a real app we would handle real registration
      ...
      if (success) {
        ...
        // Show error message
        setFormErrors({ general: 'Registration failed. Email may already be in use.' });
        setIsLoading(false);
      }
    }, 1500);
  };
    ...
    <Form method="post" onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        <div className="relative">
          <motion.label 
            htmlFor="fullName" 
            className={`absolute text-sm ${fullName ? (formErrors.fullName ? 'text-red-500 -top-6' : 'text-amber -top-6') : 'text-silver top-3 left-3'} transition-all duration-200`}
            animate={{ 
              top: fullName ? '-1.5rem' : '0.75rem',
              color: fullName ? (formErrors.fullName ? '#EF4444' : '#FFBF00') : '#C0C0C0',
              scale: fullName ? 0.85 : 1
            }}
          ...
          <input
            ...
            className={`w-full bg-[#252525] border-b-2 ${formErrors.fullName ? 'border-red-500' : 'border-silver focus:border-amber'} px-3 py-3 text-offwhite rounded-t-md outline-none transition-colors`}
            required
          />
        ...
        <div className="mt-1">
          <div className="flex justify-between mb-1 text-xs">
            ...
            <span className={`font-medium ${getPasswordStrengthTextColor(passwordStrength)}`}>
              {password ? getPasswordStrengthText(passwordStrength) : 'None'}
            </span>
          </div>
      ...
      >
        <div className="relative">
          <motion.label 
            ...
            className={`absolute text-sm ${confirmPassword ? (formErrors.confirmPassword ? 'text-red-500 -top-6' : 'text-amber -top-6') : 'text-silver top-3 left-3'} transition-all duration-200`}
            animate={{ 
              top: confirmPassword ? '-1.5rem' : '0.75rem',
              color: confirmPassword ? (formErrors.confirmPassword ? '#EF4444' : '#FFBF00') : '#C0C0C0',
              scale: confirmPassword ? 0.85 : 1
            }}
            transition={{ duration: 0.2 }}
          >
            Confirm Password
          ...
          <input
            ...
            className={`w-full bg-[#252525] border-b-2 ${formErrors.confirmPassword ? 'border-red-500' : 'border-silver focus:border-amber'} px-3 py-3 text-offwhite rounded-t-md outline-none transition-colors`}
            required
          />
          <motion.button
            type="button"
      ...
      >
        <label className="flex items-center cursor-pointer">
          ...
          />
          <motion.div 
            className={`w-4 h-4 mr-2 border ${agreeTerms ? 'bg-amber border-amber' : 'border-silver'} rounded flex items-center justify-center`}
            whileTap={{ scale: 0.9 }}
          >
            {agreeTerms && (
              <motion.svg 
          ...
          <span className="text-silver text-sm">
            I agree to the <Link to="/terms" className="text-amber hover:underline">Terms of Service</Link> and <Link to="/privacy" className="text-amber hover:underline">Privacy Policy</Link>
          </span>
        </label>
      </motion.div>
      ...
      >
        <motion.button
          type="submit"
          className={`w-full ${
            !agreeTerms || !passwordsMatch || passwordStrength < 3
              ? 'bg-gray-600 cursor-not-allowed' 
              : 'bg-amber hover:bg-amber-dark'
          } text-charcoal font-semibold py-3 px-4 rounded-md transition-colors duration-300 flex items-center justify-center`}
          whileHover={!agreeTerms || !passwordsMatch || passwordStrength < 3 ? {} : { scale: 1.02 }}
          whileTap={!agreeTerms || !passwordsMatch || passwordStrength < 3 ? {} : { scale: 0.98 }}
          disabled={isLoading || !agreeTerms || !passwordsMatch || passwordStrength < 3}
        >
          {isLoading ? (
            <motion.div
      ...
      >
        <div className="relative flex items-center justify-center">
          ...
          <div className="absolute bg-[#1A1A1A] px-4 text-sm text-silver">or sign up with</div>
        </div>
